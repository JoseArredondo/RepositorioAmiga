Public Class WbFindbcReimpresion
...
#End Region
    ...
    Public Event Reimpresion(ByVal ccodcta As String, ByVal cnomcli As String, ByVal fecha As String)
...
#Region " Metodos de la Pagina "
    Private Sub Cargardatos()
        ...
        Dim i As Integer
        Dim clsCremcre As New ccreditos

        'ds = clsCli.BuscaClienteGru(Me.TxtNombre.Text.Trim, Me.cbxcentros.SelectedValue.Trim, "02", Session("gcCodofi"))
        ds = clsCremcre.ObtenerbusquedacreditoGrupales(Me.TxtNombre.Text.Trim, "", "1", "4", "2", "1", Session("gcCodofi"))
        i = ds.Tables(0).Rows.Count()
        If i >= 1 Then
            Me.dgClientes.DataSource = ds.Tables(0)
            Me.dgClientes.DataBind()
...
#End Region
    ...
    Private Sub dgClientes_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dgClientes.SelectedIndexChanged
        ...
        Dim fila As String
        fila = dgClientes.SelectedIndex.ToString

        ccodcta = CType(Me.dgClientes.SelectedItem.FindControl("Hyperlink1"), HyperLink).Text.Trim
        cnomcli = dgClientes.Items(fila).Cells(2).Text.ToString.Trim
        dfecha = dgClientes.Items(fila).Cells(4).Text.ToString.Trim
        RaiseEvent Reimpresion(ccodcta, cnomcli, dfecha)
