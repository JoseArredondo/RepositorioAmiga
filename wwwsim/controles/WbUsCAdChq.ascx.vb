...
#Region " Metodos "
    ...
    Public Sub CargarNoPartida(ByVal codigoPartida As String)
        ...
        Else
            ...
            Catch ex As Exception
                ...
                Response.Write("<script language='javascript'>alert('Banco Para Usuario Inaccesible')</script>")
            End Try
    ...
    Public Sub Imprime()
        ...
        Dim lcnomlug As String = Session("gcnomofi")
        Dim lcnrochq As String = Me.TxtNoChq.Text.Trim
        lcletras = clsConvert.ConversionEnteros(Double.Parse(Me.TxtMonChq.Text.Trim))
        lndecimal = clsConvert.ExtraeDecimales(Me.TxtMonChq.Text.Trim)
        ...
        Try
            ...
            crRpt.Load(Server.MapPath("reportes") + "\RptCheques.rpt", OpenReportMethod.OpenReportByDefault)
        ...
        Dim lccodusu As String = Session("gccodusu")
        'Crear aqui el dataset

        dsDes = cClsAdP.Reporte(Me.TxtNumpar.Text.Trim)
        ...
        Else
            ...
            Dim fila As DataRow
            For Each fila In dsDes.Tables(0).Rows
                dsDes.Tables(0).Rows(nelem)("cmonlet") = lcletras
                lccodusu = fila("ccodusu")
                nelem += 1
            Next
        End If
        Dim eusuarios As New cusuarios
        Dim lccodofi As String = ""
        lccodofi = eusuarios.Oficina(lccodusu)
        ' Setear los registros recuperados, diciendole el Table respectivo
        crRpt.SetDataSource(dsDes.Tables(0))


        crRpt.Refresh()
        crRpt.SetParameterValue("cnomlug", lcnomlug.Trim)
        crRpt.SetParameterValue("cnumpar", "PARTIDA Nº: " & lcnumpar)
        crRpt.SetParameterValue("cnrochq", "Cheque: " & lcnrochq)
        crRpt.SetParameterValue("pccodofi", lccodofi)
...
#End Region
    ...
    Private Sub TxtMonChq_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtMonChq.TextChanged
        Me.RangeValidator2.Validate()
        If Me.RangeValidator2.IsValid = False Then
            Exit Sub
        End If
        Dim lnDecimal As Double
        Me.TxtLetras.Text = clsConvert.ConversionEnteros(Double.Parse(Me.TxtMonChq.Text.Trim))
        lnDecimal = clsConvert.ExtraeDecimales(Me.TxtMonChq.Text.Trim)
        Me.TxtLetras.Text = Me.TxtLetras.Text.Trim & " " & lnDecimal.ToString & "/100"
        TxtnHaber.Text = TxtMonChq.Text
    End Sub

    Private Sub TxtLetras_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TxtLetras.TextChanged
    ...
    Private Sub btnProcesa_ServerClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProcesa.ServerClick
        ...
        If (Me.TxtnDebe.Text.Trim = " " Or Me.TxtnDebe.Text.Trim = Nothing) And Me.TxtnDebe.Text.Trim <> "0" And Me.TxtnDebe.Text.Trim <> "0.00" Then
            ...
            End If
        End If
        'Valida que cuando cuentas sea de banco concuerde con monto del cheque

        If Txtccodcta.Text.Trim.Length >= 5 Then
            If Left(Txtccodcta.Text.Trim, 5) = "11102" Then
                Try
                    If Math.Round(Double.Parse(TxtnHaber.Text), 2) <> Math.Round(Double.Parse(TxtMonChq.Text), 2) Then
                        Response.Write("<script language='javascript'>alert('Monto a Abonar de Cuenta Bancaria diferente a Monto de Cheque')</script>")
                        Exit Sub
                    End If
                Catch ex As Exception
    ...
    Private Sub btnsave_ServerClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnsave.ServerClick
        ...
        If Me.TxtNomUsu.Text.Trim = "" Or Me.TxtNomUsu.Text.Trim = Nothing Or _
            Me.TxtGlosa.Text.Trim = "" Or Me.TxtGlosa.Text.Trim = Nothing Then
        ...
        If Me.TxtMonChq.Text.Trim = Nothing Or _
                Me.TxtMonChq.Text.Trim = "" Or Me.TxtNoChq.Text.Trim = Nothing Or _
                Me.TxtNoChq.Text.Trim = "" Then
            Response.Write("<script language='javascript'>alert('Se dejan espacios vacios, Revise')</script>")
            Exit Sub
        End If
        ...
        cClsAdP._cpoliza = "EG"
        cClsAdP._nCuenta = 1
        cClsAdP._cnumdoc = Me.TxtNoChq.Text.Trim
    ...
    Private Sub cargabancos()
        ...
        Me.TxtnHaber.Text = Me.TxtMonChq.Text

        'Me.Txtccodcta_TextChanged(sender, e)
        Me.CargarCuenta(Me.Txtccodcta.Text.Trim)
    End Sub
    ...
    Public Sub ImprimeEspecial()
        Dim miclase As New clase_especial
        Dim miclase1 As New clase_funciones


        Dim str_select As String
        str_select = "SELECT cntamov.cNumcom, Diario.cGlosa, cntamov.dfeccnt, cntamov.ffondos, cntamov.ccodofi, " & _
        "ctbdchq.cnomchq, ctbmcta.ccodcta, ctbmcta.cdescrip, cntamov.ndebe, cntamov.nhaber, ctbdchq.cmonchq, ctbdchq.ccodbco," & _
        "ctbdchq.cctacte, ctbdchq.cmonlet, ctbdchq.cnrochq, cntamov.ffondos, Tabtbco.cnombco," & _
        "cntamov.ccodofi FROM ctbmcta INNER JOIN cntamov ON ctbmcta.ccodcta = cntamov.ccodcta INNER JOIN Diario " & _
        "ON Cntamov.cnumcom = Diario.cnumcom INNER JOIN CTBDCHQ ON Diario.cnumcom = Ctbdchq.cnumcom " & _
        "INNER JOIN TABTBCO ON Tabtbco.cCodbco = ctbdchq.cCodbco " & _
        "WHERE ctbdchq.cflc = ' ' and cntamov.cflc = ' ' and " & _
        "cntamov.cNumcom = " & miclase1.ToString(Me.TxtNumpar.Text.Trim)

        Session("str_select") = str_select
        Response.Redirect("~/reporte_contenedor.aspx?parametro=" & "cheque")
