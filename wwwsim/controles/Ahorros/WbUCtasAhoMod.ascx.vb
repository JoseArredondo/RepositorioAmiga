...
Partial Class controles_Ahorros_WbUCtasAhoMod
    Inherits ucWBase
...
#Region "Metodos"
    ...
    Private Sub Buscar_Asociado()
        ...
        ds_find = occlass_aho.Extraer_Datos_Ctas_Aho(lntipo, Me.TxtcEvalua.Text.Trim)
...
#End Region
    ...
    Protected Sub btnproc_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnproc.Click
        ...
        If Me.TxtcNomben.Text.Trim.Length = 0 Then
            codigoJs = "<script language='javascript'>alert('Nombre del Beneficiario esta vacia, " & _
                    "Advertencia SIM.NET ')</script>"
            ScriptManager.RegisterStartupScript(Me.Page(), Me.GetType(), "Ventana_Aviso", codigoJs, False)
        ...
        If Me.TxtcDirBen.Text.Trim.Length = 0 Then
            codigoJs = "<script language='javascript'>alert('La Dirección del Beneficiario esta vacia, " & _
                    "Advertencia SIM.NET ')</script>"
            ScriptManager.RegisterStartupScript(Me.Page(), Me.GetType(), "Ventana_Aviso", codigoJs, False)
    ...
    Protected Sub btnsave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnsave.Click
        ...
        'Valida si porcentaje supera el 100%
        'If Not IsDBNull(vDetalle.Tables("Beneficiarios").Compute("sum(nPorcen)", "")) Then
        '    pnTotpor = vDetalle.Tables("Beneficiarios").Compute("sum(nPorcen)", "")
        'End If
    ...
    Protected Sub Gridfind_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Gridfind.SelectedIndexChanged
        ...
        entidadahomcta.ccodaho = Me.Gridfind.SelectedRow.Cells(2).Text
        omahomcta.ObtenerAhomcta(entidadahomcta)

        Me.CbxTipAho1.SelectedValue = entidadahomcta.producto.Trim
        CbxLinAhoProd1.Recuperar(Me.CbxTipAho1.SelectedValue.Trim)
        Me.CbxLinAhoProd1.SelectedValue = entidadahomcta.cnrolin.Trim
        Me.Txtnlibreta.Text = entidadahomcta.nlibreta

        'Cheque_Seleccionado(Me.Gridfind.SelectedRow.Cells(1).Text)
        Me.TxtcCodcli.Text = Me.Gridfind.SelectedRow.Cells(1).Text
        Me.TxtcCodaho.Text = Me.Gridfind.SelectedRow.Cells(2).Text
        Me.TxtcNombres.Text = Me.Gridfind.SelectedRow.Cells(3).Text
        Me.TxtcApellidos.Text = Me.Gridfind.SelectedRow.Cells(4).Text
        ...
        'Extre firmas y Beneficiarios
        vDetalle = cClsAho.Extraer_Datos_Ctas_Detalle(Me.TxtcCodaho.Text.Trim)


        ViewState("Dataset") = vDetalle

        GridBen.DataSource = vDetalle.Tables("Beneficiarios")
        GridBen.DataBind()

        GridFirm.DataSource = vDetalle.Tables("Firmas")
        GridFirm.DataBind()
    ...
    Protected Sub btnproc0_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnproc0.Click
        ...
        If Me.TxtcNomFir.Text.Trim.Length = 0 Then
            codigoJs = "<script language='javascript'>alert('Nombre de la Firma vacia, " & _
                    "Advertencia SIM.NET ')</script>"
            'ScriptManager.RegisterStartupScript(Me.Page(), Me.GetType(), "Ventana_Aviso", codigoJs, False)
        ...
        If Me.Txtdnacimi0.Text.Trim.Length = 0 Then
            codigoJs = "<script language='javascript'>alert('La fecha de nacimiento esta vacia, " & _
                    "Advertencia SIM.NET ')</script>"
            ScriptManager.RegisterStartupScript(Me.Page(), Me.GetType(), "Ventana_Aviso", codigoJs, False)
        ...
        If Me.TxtcnudociFir.Text.Trim.Length = 0 Then
            codigoJs = "<script language='javascript'>alert('Numero de DUI vacio, " & _
                    "Advertencia SIM.NET ')</script>"
            ScriptManager.RegisterStartupScript(Me.Page(), Me.GetType(), "Ventana_Aviso", codigoJs, False)
        ...
        If Not IsDate(Me.Txtdnacimi0.Text) Then
            codigoJs = "<script language='javascript'>alert('La fecha no es valida, " & _
                    "Advertencia SIM.NET ')</script>"
            ScriptManager.RegisterStartupScript(Me.Page(), Me.GetType(), "Ventana_Aviso", codigoJs, False)
    ...
    Private Sub ImpresionRegFirmas()
        Dim crRpt As New ReportDocument
        Dim rptStream As New System.IO.MemoryStream

        Try
            crRpt.Load(Server.MapPath("reportes") + "\crRegFirmas.rpt", OpenReportMethod.OpenReportByDefault)
        Catch ex As Exception
            Return
        End Try

        ' Recuperar los Beneficiarios
        Dim dt As New DataTable
        Dim dr As DataRow
        dt.Columns.Add("cnomcli")
        dt.Columns.Add("nporcen")
        dt.Columns.Add("cdirben")

        For i As Integer = 0 To GridBen.Rows.Count - 1
            dr = dt.NewRow()
            dr("cnomcli") = GridBen.Rows(i).Cells(2).Text
            dr("nporcen") = GridBen.Rows(i).Cells(6).Text
            dr("cdirben") = GridBen.Rows(i).Cells(5).Text
            dt.Rows.Add(dr)
        Next

        ' Recuperar las Firmas
        Dim dtF As New DataTable
        Dim drF As DataRow
        dtF.Columns.Add("cnomfirm1")
        dtF.Columns.Add("cduifirm1")
        dtF.Columns.Add("cnomfirm2")
        dtF.Columns.Add("cduifirm2")
        dtF.Columns.Add("cnomfirm3")
        dtF.Columns.Add("cduifirm3")

        drF = dtF.NewRow()
        For i As Integer = 0 To GridFirm.Rows.Count - 1
            drF("cnomfirm" & (i + 1).ToString()) = GridFirm.Rows(i).Cells(2).Text.Trim
            drF("cduifirm" & (i + 1).ToString()) = GridFirm.Rows(i).Cells(4).Text.Trim
        Next
        dtF.Rows.Add(drF)

        Dim eClimide As New climide
        Dim mClimide As New cClimide
        Dim entidadpagaduria As New pagadurias
        Dim epagadurias As New cPagadurias
        ...
        Dim lcpagaduria As String = ""
        Dim lcprof As String = ""
        Dim ccodofi As String = ""
        Dim usuario As String = Session("gccodusu")
        Dim clugprint As String = ""

        eClimide.ccodcli = TxtcCodcli.Text.Trim
        mClimide.ObtenerClimide(eClimide)

        'entidadpagaduria.IdEmp = eClimide.cLtrab
        'epagadurias.ObtenerPagadurias(entidadpagaduria)
        'lcpagaduria = entidadpagaduria.nombre

        lcpagaduria = ""

        entidadprf.ccodigo = eClimide.ccodpro
        etabtprf.ObtenerTabtprf(entidadprf)
        lcprof = entidadprf.cdescri.Trim

        ccodofi = mUsuarios.Oficina(usuario)
        eTabtofi.ccodofi = ccodofi
        mTabtofi.ObtenerTabtofi(eTabtofi)

        clugprint = eTabtofi.cmuni.ToUpper & CDate(Session("gdfecsis")).ToString("D")

        ' Agregando el table
        crRpt.SetDataSource(dt)
        crRpt.Refresh()

        crRpt.SetParameterValue("cnomcli", Txtctitular.Text.Trim)
        crRpt.SetParameterValue("ccodaho", TxtcCodaho.Text.Trim)
        crRpt.SetParameterValue("cdirdom", eClimide.cdirdom)
        crRpt.SetParameterValue("ctelefono", eClimide.cteldom)
        crRpt.SetParameterValue("cprof", lcprof)
        crRpt.SetParameterValue("cdui", eClimide.cnudoci)
        crRpt.SetParameterValue("nedad", (CDate(eClimide.dfecnac).Year - Date.Now.Year).ToString())
        crRpt.SetParameterValue("clugtra", lcpagaduria)
        crRpt.SetParameterValue("ccodcli", eClimide.ccodcli)
        crRpt.SetParameterValue("cnomfirm1", dtF.Rows(0)("cnomfirm1").ToString())
        crRpt.SetParameterValue("cduifirm1", dtF.Rows(0)("cduifirm1").ToString())
        crRpt.SetParameterValue("cnomfirm2", dtF.Rows(0)("cnomfirm2").ToString())
        crRpt.SetParameterValue("cduifirm2", dtF.Rows(0)("cduifirm2").ToString())
        crRpt.SetParameterValue("cnomfirm3", dtF.Rows(0)("cnomfirm3").ToString())
        crRpt.SetParameterValue("cduifirm3", dtF.Rows(0)("cduifirm3").ToString())
        crRpt.SetParameterValue("clugprint", clugprint)

        Dim reportes As String = "reporte"
